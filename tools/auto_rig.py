...
# ================================================================
# PHYSICS-BASED ARTHROPOD WEIGHT ASSIGNMENT
#
# From arachnid biomechanics:
# - Chitin exoskeleton = RIGID PLATES → weight 1.0 to local bone
# - Articular membranes = NARROW HINGES → linear blend at joints
# - No other tissue → no cross-segment influence
#
# This is a step function with linear ramps at joints.
# Not a heuristic. This is how the material behaves.
# ================================================================
...
lines.append('')
lines.append('    print("Computing physics-based arthropod weights...")')
lines.append('    print("  Chitin plates: rigid (weight 1.0)")')
lines.append('    print("  Articular membranes: linear blend at joints")')
lines.append('    bpy.context.view_layer.objects.active = mesh')
lines.append('')
lines.append('    JOINT_BLEND_WIDTH = 0.02  # meters — width of articular membrane')
lines.append('')
lines.append('    # Build bone chain data')
lines.append('    bone_data = []  # [(name, head_world, tail_world, group, chain_index)]')
lines.append('    bone_chains = {}  # group -> ordered list of bone names')
...
lines.append('        """Project point p onto bone segment a→b. Returns (distance, t_parameter)."""')
lines.append('        ab = b - a')
lines.append('        ab_len_sq = ab.dot(ab)')
lines.append('        if ab_len_sq < 1e-10:')
lines.append('            return (p - a).length, 0.5')
lines.append('        t = max(0, min(1, (p - a).dot(ab) / ab_len_sq))')
...
lines.append('')
lines.append('        # Step 1: Find closest bone per group, with body priority')
lines.append('        all_dists = []')
lines.append('        body_dist = float("inf")')
lines.append('        body_bone = None')
lines.append('        body_t = 0')
lines.append('        for bname, bh, bt, bgroup, cidx in bone_data:')
lines.append('            d, t = project_onto_bone(v_world, bh, bt)')
lines.append('            all_dists.append((d, bname, bgroup, cidx, t))')
lines.append('            if bgroup == "body" and d < body_dist:')
lines.append('                body_dist = d')
lines.append('                body_bone = bname')
lines.append('                body_t = t')
lines.append('')
lines.append('        all_dists.sort()')
lines.append('        best_dist, best_bone, best_group, best_chain_idx, best_t = all_dists[0]')
lines.append('')
lines.append('        # BODY PRIORITY: the cephalothorax and abdomen are LARGE rigid plates.')
lines.append('        # Any vertex within 0.12m of a body bone is body, period.')
lines.append('        # Otherwise, body wins if within 5x the closest bone distance.')
lines.append('        if best_group != "body" and body_bone and (body_dist < 0.12 or body_dist < best_dist * 5.0):')
lines.append('            best_bone = body_bone')
lines.append('            best_group = "body"')
lines.append('            best_chain_idx = 0')
lines.append('            best_t = body_t')
lines.append('            best_dist = body_dist')
lines.append('')
lines.append('        # Step 2: Rigid plate or joint membrane?')
lines.append('        chain = bone_chains[best_group]')
lines.append('        bone_head = None')
lines.append('        bone_tail = None')
lines.append('        for bname, bh, bt, bg, ci in bone_data:')
lines.append('            if bname == best_bone:')
lines.append('                bone_head = bh')
lines.append('                bone_tail = bt')
lines.append('                break')
...
lines.append('')
lines.append('        # Check if near the HEAD of the bone (joint with previous bone in chain)')
lines.append('        if best_t < blend_t and best_chain_idx > 0:')
lines.append('            prev_bone = chain[best_chain_idx - 1]')
lines.append('            factor = best_t / blend_t  # 0 at joint, 1 at blend edge')
lines.append('            weights[best_bone] = factor')
lines.append('            weights[prev_bone] = 1.0 - factor')
lines.append('')
lines.append('        # Check if near the TAIL of the bone (joint with next bone in chain)')
lines.append('        elif best_t > (1.0 - blend_t) and best_chain_idx < len(chain) - 1:')
lines.append('            next_bone = chain[best_chain_idx + 1]')
lines.append('            factor = (1.0 - best_t) / blend_t  # 0 at joint, 1 at blend edge')
lines.append('            weights[best_bone] = factor')
lines.append('            weights[next_bone] = 1.0 - factor')
lines.append('')
lines.append('        # Check if coxa near body (blend with cephalothorax)')
lines.append('        elif best_t > (1.0 - blend_t) and best_chain_idx == len(chain) - 1 and best_group.startswith("leg_"):')
lines.append('            factor = (1.0 - best_t) / blend_t')
lines.append('            weights[best_bone] = factor')
lines.append('            weights["cephalothorax"] = 1.0 - factor')
lines.append('')
lines.append('        else:')
lines.append('            # RIGID PLATE — 100% to this bone')
lines.append('            weights[best_bone] = 1.0')
lines.append('')
lines.append('        # Assign weights')
...
lines.append('    print("Physics-based arthropod weights:")')
lines.append('    print("  Rigid plates + linear blend at joints")')
lines.append('    print(f"  Bones: {len(arm.data.bones)}")')
lines.append('    print(f"  Joint blend width: {JOINT_BLEND_WIDTH}m")')
lines.append('    print("=" * 50)')
lines.append('')
lines.append('try:')
lines.append('    build()')
