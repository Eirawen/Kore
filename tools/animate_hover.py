...
# ═══════════════ 3. SIMULATED FLIGHT (not eased, not even ballistic) ══
# A smooth hover reads as levitation. A hover that SAGS BETWEEN BEATS reads
# as WORK: each downstroke arrests the fall and buys back altitude, then
# gravity takes it during the recovery stroke. She is fighting gravity and
# barely losing — the vestigial-wing story told as a trajectory.
#
# So height is INTEGRATED, never keyed by feel:
#   v -= g*dt every step;  v += impulse at each downstroke
# Two things fall out for free: the sawtooth sag, and CAUSALITY — the
# impulse fires mid-downstroke, so she starts rising 2 frames AFTER the
# wings move. Force leads position. The LANDING FRAME is an OUTPUT.
G = 9.81
JUMP_H = 0.34
TAKEOFF_F = 40
...
FLAP_PERIOD = 11                  # small wings must beat FAST
N_FLAPS = 4
FLAP_LIFT = 1.52                  # m/s per downstroke — deliberately LESS
                                  # than g*period (1.80), so she loses a
                                  # little altitude every beat
APEX_F = TAKEOFF_F + int(round(V0 / G * FPS))
FLAPS = [APEX_F + i * FLAP_PERIOD for i in range(N_FLAPS)]
IMPULSE_AT = {f + 2: FLAP_LIFT for f in FLAPS}   # mid-downstroke

def simulate():
    z = TAKEOFF_OFF / 100.0
    v = V0
    h = {TAKEOFF_F: TAKEOFF_OFF}
    f = TAKEOFF_F
    while f < TAKEOFF_F + 400:
        f += 1
        if f in IMPULSE_AT:
            v += IMPULSE_AT[f]
        v -= G / FPS
        z += v / FPS
        h[f] = z * 100.0
        if v < 0 and h[f] <= LAND_OFF:
            return h, f
    return h, f

HEIGHT, LAND_F = simulate()
peak_f = max(HEIGHT, key=lambda k: HEIGHT[k])
print('FLIGHT v0=%.2fm/s apex_f=%d flaps_at=%s lift=%.2fm/s (g*period=%.2f)'
      % (V0, APEX_F, FLAPS, FLAP_LIFT, G * FLAP_PERIOD / FPS))
print('FLIGHT peak %.1fcm @f%d   LAND_F=%d (simulated, %d frames airborne)'
      % (HEIGHT[peak_f], peak_f, LAND_F, LAND_F - TAKEOFF_F))
sag = [(f, round(HEIGHT[f], 1)) for f in sorted(HEIGHT) if f % 4 == 0]
print('FLIGHT profile(cm) %s' % sag)
...
# the loaded top of the stroke — a flap needs something to beat DOWN from
WING_UP = {
    'WingL_root': [(Y, 60)], 'WingR_root': [(Y, -60)],
    'WingL_tip': [(Y, 54)], 'WingR_tip': [(Y, -54)],
}
# recovery: partly folded so the upstroke sheds drag (real flapping)
WING_RECOVER = {
    'WingL_root': [(Y, 34)], 'WingR_root': [(Y, -34)],
    'WingL_tip': [(Y, 12)], 'WingR_tip': [(Y, -12)],
}
LAUNCH_BODY = {
...
DANGLE = {
    'LeftUpLeg': ('axis', tuple(AX['LeftUpLeg']), 18),
    'RightUpLeg': ('axis', tuple(AX['RightUpLeg']), 14),
    'LeftLeg': ('axis', tuple(AX['LeftLeg']), 24),
    'RightLeg': ('axis', tuple(AX['RightLeg']), 20),
    'LeftFoot': ('axis', tuple(AX['LeftFoot']), -30),
    'RightFoot': ('axis', tuple(AX['RightFoot']), -30),
}
EXTEND = {
...
# legs DANGLE through the hover and LAG the bob — they are along for the
# ride, not helping. Offset from each flap so they trail it.
for _i, _ff in enumerate(FLAPS):
    key_comp(_ff + 5, 'legs', TUCK if _i % 2 == 0 else DANGLE)
    key_comp(_ff + 10, 'legs', DANGLE if _i % 2 == 0 else TUCK)
key_comp(LAND_F - 8, 'legs', REACH)               # reaching for the floor
...
key_comp(APEX_F, 'spine', AIR_BODY)
for _ff in FLAPS:
    key_comp(_ff + 4, 'spine', AIR_BODY, 1.10)    # torso braces on the beat
    key_comp(_ff + 9, 'spine', AIR_BODY, 0.94)    # and eases on recovery
...
key_comp(FLAPS[0] - 4, 'wings', WING_UP)          # cocks up ready to beat
for _i, _ff in enumerate(FLAPS):
    key_comp(_ff, 'wings', WING_UP)               # top of the stroke
    key_comp(_ff + 4, 'wings', FLAP_DOWN)         # POWER — impulse at +2
    key_comp(_ff + 8, 'wings', WING_RECOVER)      # shed drag on the way up
key_comp(FLAPS[-1] + 12, 'wings', AIR_BODY)       # spent: a tired glide
...
prev = None
for f in ([1, 30, TAKEOFF_F, APEX_F] + [x + 4 for x in FLAPS]
          + [LAND_F - 6, LAND_F, LAND_F + 12, FRAMES]):
    scene.frame_set(f)
