...
for i, (dv, fr, mu, label) in enumerate(VIEWS):
    shoot(dv, fr, mu, OUT + '\\coy4chk_%02d.png' % (i + 1))
    man.append({'index': i + 1, 'label': label})
with open(OUT + '\\coy4chk_manifest.json', 'w') as fh:
    json.dump({'samples': man}, fh)
print('GATE rendered')

# ───────────────── the motion: slow / quick / STALL / slowest ────────
FRAMES = 168
if '--grid' in ARGS or '--full' in ARGS:
    ...
    key_rest(1, allb)
    # f9: the shy FLINCH away — fast, a reflex
    apply_pose(9, FLINCH)
    key_rest(9, [b for b in COY if b not in FLINCH])
    # f30: the body starts contracting; arm begins to rise
    apply_pose(30, {k: v for k, v in COY.items()
                    if k not in ('Head', 'neck')}, 0.45)
    apply_pose(30, FLINCH)
    # f46: hand nearly there, shoulder rising to meet it
    apply_pose(46, {k: v for k, v in COY.items()
                    if k not in ('Head', 'neck')}, 0.82)
    apply_pose(46, FLINCH)
    # f52-58: HESITATION STALL — she almost doesn't
    apply_pose(52, {k: v for k, v in COY.items()
                    if k not in ('Head', 'neck')}, 0.86)
    apply_pose(58, {k: v for k, v in COY.items()
                    if k not in ('Head', 'neck')}, 0.88)
    apply_pose(52, FLINCH)
    apply_pose(58, FLINCH)
    # f68: hand ARRIVES, shoulder squeezes (slight overshoot)
    apply_pose(68, OVER)
    apply_pose(68, FLINCH)
    # f78: body settles
    apply_pose(78, {k: v for k, v in COY.items() if k not in ('Head', 'neck')})
    apply_pose(78, FLINCH)
    # f78 -> f108: the SLOW LOOK BACK. the slowest move in the piece.
    apply_pose(108, {'neck': COY['neck'], 'Head': COY['Head']})
    # living hold: small (coy people are still) + one micro-fidget
    for f in range(120, FRAMES + 1, 12):
        t = (f - 108) / 60.0
        sway = math.sin(t * math.pi * 0.7) * 0.9
        breath = math.sin(t * math.pi * 1.5) * 0.6
        fidget = 1.6 if 138 <= f <= 150 else 0.0     # a shy weight shift
        hold = dict(COY)
        hold['Hips'] = BODY['Hips'] + [(Y, sway + fidget)]
        hold['Spine01'] = BODY['Spine01'] + [(X, breath)]
        hold['Spine'] = BODY['Spine'] + [(X, breath * 0.7)]
        apply_pose(f, hold)
    ...
    if '--grid' in ARGS:
        scene.render.resolution_x, scene.render.resolution_y = 480, 560
        frames = sorted({max(1, min(FRAMES, round(1 + (FRAMES - 1) * i / 11)))
                         for i in range(12)})
        gm = []
        for i, f in enumerate(frames):
            scene.frame_set(f)
            shoot(HERO, 'full', 1.5, OUT + '\\coy4grid_%02d.png' % (i + 1))
            gm.append({'index': i + 1, 'frame': f,
                       'time': round((f - 1) / FPS, 3)})
        with open(OUT + '\\coy4grid_manifest.json', 'w') as fh:
    ...
    if '--full' in ARGS:
        ...
        scene.render.filepath = OUT + '\\coy4full_'
