...
if '--gate' in ARGS:
    ...
    for i, (dv, fr, mu, label) in enumerate(VIEWS):
        shoot(dv, fr, mu, OUT + '\\coy6chk_%02d.png' % (i + 1))
        man.append({'index': i + 1, 'label': label})
    with open(OUT + '\\coy6chk_manifest.json', 'w') as fh:
...
# ── HEAD ────────────────────────────────────────────────────────────
# The flinch is a REFLEX: fast (12f), then it RELAXES slightly — a snap
# with recovery, not a hard stop against a wall.
key_comp(5, 'head', FLINCH, 0.26)     # rise time: even a reflex ramps
key_comp(15, 'head', FLINCH, 1.0)
key_comp(27, 'head', FLINCH, 0.93)
key_comp(58, 'head', FLINCH, 0.95)
# THE LOOK BACK starts at f58 — while the hand is still RISING (it lands
# at f96). Khaled's note: "as it approaches, she's already turning to
# you." Profile is reluctant-then-committing: barely moves at first,
# commits through the middle, settles late.
key_comp(82, 'head', COY, 0.18)      # reluctance — a hair of movement
key_comp(108, 'head', COY, 0.58)     # commits
key_comp(140, 'head', COY, 1.00)     # settles

# ── CHEST: lags the head into the turn; breath holds, then exhales ───
key_comp(18, 'chest', COY, 0.5)
key_comp(38, 'chest', COY, 1.0)
BR = lambda a: {'Spine01': [(X, a)], 'Spine': [(X, a * 0.7)],
                'Spine02': [(X, a * 0.5)]}
key_comp(50, 'chest', COY, 1.0, extra=BR(0.9))
key_comp(66, 'chest', COY, 1.0, extra=BR(-0.6))
key_comp(84, 'chest', COY, 1.0, extra=BR(-0.7))   # held through the approach
key_comp(104, 'chest', COY, 1.0, extra=BR(1.5))   # the exhale after contact
key_comp(124, 'chest', COY, 1.0, extra=BR(0.2))

# ── POSTURAL LAYER: slow, early, out of the way ─────────────────────
key_comp(20, 'pelvis', COY, 0.35)
key_comp(58, 'pelvis', COY, 1.0)
key_comp(22, 'legs', COY, 0.3)
key_comp(62, 'legs', COY, 1.0)
key_comp(24, 'feet', COY, 0.3)
key_comp(64, 'feet', COY, 1.0)

# ── RIGHT ARM: protective, automatic, never hesitates. Decelerating
# arrival (fast commit, soft landing) — 4 keys so it doesn't step.
key_comp(14, 'rarm', COY, 0.22)
key_comp(30, 'rarm', COY, 0.70)
key_comp(46, 'rarm', COY, 0.93)
key_comp(62, 'rarm', COY, 1.00)

# ── LEFT ARM: the self-conscious one. DECELERATING approach — the
# hesitation is a SLOWING, never a stop (a dead key reads as a paused
# game; that was the jolt). Increments per frame:
#   f34-56: .021   f56-74: .013   f74-84: .005 (the hesitation)  f84-96: .008
# No overshoot: a hand landing on your own face is damped by contact.
key_comp(34, 'larm', COY, 0.16)
key_comp(56, 'larm', COY, 0.62)
key_comp(74, 'larm', COY, 0.86)
key_comp(84, 'larm', COY, 0.91)      # ── hesitation: still moving, barely
key_comp(96, 'larm', COY, 1.00)      # creeps in and rests

# ── LEFT SHOULDER: the squeeze is a CONSEQUENCE of the hand arriving,
# so it trails it and eases in long after.
key_comp(46, 'lshoulder', COY, 0.22)
key_comp(96, 'lshoulder', COY, 0.60)
key_comp(128, 'lshoulder', COY, 1.00)

# ════════════════ THE HOLD ════════════════
key_comp(160, 'chest', COY, 1.0, extra=BR(-0.6))
key_comp(188, 'chest', COY, 1.0, extra=BR(0.8))
key_comp(210, 'chest', COY, 1.0, extra=BR(-0.2))

# the shy WEIGHT SHIFT: one event, slower and gentler than before, and
# only the postural components move. It resolves and stays resolved.
SHIFT_P = {'Hips': COY['Hips'] + [(Y, 1.2)],
           'HIPS_LOC': (COY['HIPS_LOC'][0] + 0.4, COY['HIPS_LOC'][1],
                        COY['HIPS_LOC'][2] + 0.5)}
SHIFT_L = {'LeftUpLeg': COY['LeftUpLeg'] + [(X, -2.0)],
           'LeftLeg': COY['LeftLeg'] + [(X, 2.4)],
           'RightUpLeg': COY['RightUpLeg'], 'RightLeg': COY['RightLeg']}
SHIFT_F = {'LeftFoot': COY['LeftFoot'] + [(Z, 6)],
           'RightFoot': COY['RightFoot']}
key_comp(152, 'pelvis', COY, 1.0)
key_comp(176, 'pelvis', SHIFT_P, 1.0)
key_comp(210, 'pelvis', SHIFT_P, 1.0)
key_comp(156, 'legs', COY, 1.0)
key_comp(180, 'legs', SHIFT_L, 1.0)
key_comp(210, 'legs', SHIFT_L, 1.0)
key_comp(162, 'feet', COY, 1.0)
key_comp(186, 'feet', SHIFT_F, 1.0)
key_comp(210, 'feet', SHIFT_F, 1.0)

key_comp(172, 'head', COY, 1.0, extra={'neck': [(Z, -0.5)]})
key_comp(204, 'head', COY, 1.0, extra={'neck': [(Z, 0.35)]})
key_comp(210, 'larm', COY, 1.0)
key_comp(210, 'lshoulder', COY, 1.0)
key_comp(210, 'rarm', COY, 1.0)

# ── HANDLES: the other half of the jolt. AUTO_CLAMPED on EVERY key
# flattens velocity to zero AT every key, so a multi-key move becomes a
# chain of little stop-starts (the "stepping"). Clamp only the true
# endpoints; give pass-through keys AUTO so velocity carries through.
        ...
        for fc in fcs:
            ...
            for i, kp in enumerate(kps):
                ...
                if i == 0 or i == last:
                    ...
                    nm_ += 1
    print('HANDLES %d clamped endpoints, %d smooth pass-through' % (nk, nm_))
scene.frame_start, scene.frame_end = 1, FRAMES

# ── numeric smoothness audit: dead frames, jolts, component overlap ──
if '--velcheck' in ARGS or True:
    import importlib.util as _il
    _sp = _il.spec_from_file_location(
        'vc', '/home/khaled/Kore/tools/velocity_check.py')
    _vc = _il.module_from_spec(_sp)
    _sp.loader.exec_module(_vc)
    spans = {}
    for lbl, bones in (('L hand', ['LeftHand']), ('R hand', ['RightHand']),
                       ('head', ['headfront']), ('pelvis', ['Hips']),
                       ('L foot', ['LeftFoot'])):
        spans[lbl] = _vc.report(arm, scene, bones, 1, FRAMES, lbl)
    lh, hd = spans.get('L hand'), spans.get('head')
    if lh and hd:
        ov = min(lh[1], hd[1]) - max(lh[0], hd[0])
        print('OVERLAP L-hand vs head-turn = %d frames (%s)'
              % (ov, 'overlapping' if ov > 10 else 'SEQUENTIAL'))
    lh, rh = spans.get('L hand'), spans.get('R hand')
    if lh and rh:
        print('ASYMMETRY L-arm f%d-%d vs R-arm f%d-%d  (start gap %df, '
              'end gap %df)' % (lh[0], lh[1], rh[0], rh[1],
                                lh[0] - rh[0], lh[1] - rh[1]))
...
if '--grid' in ARGS:
    ...
    for i, f in enumerate(frames):
        scene.frame_set(f)
        shoot(HERO, 'full', 1.5, OUT + '\\coy6grid_%02d.png' % (i + 1))
        gm.append({'index': i + 1, 'frame': f, 'time': round((f - 1) / FPS, 3)})
    with open(OUT + '\\coy6grid_manifest.json', 'w') as fh:
...
if '--full' in ARGS:
    ...
    scene.render.filepath = OUT + '\\coy6full_'
